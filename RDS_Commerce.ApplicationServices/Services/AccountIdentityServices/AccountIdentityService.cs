...
namespace RDS_Commerce.ApplicationServices.Services.AccountIdentityServices;
public class AccountIdentityService : IAccountIdentityService
{
    ...
    public async Task<bool> CreateIdentityAccountAsync(AccountIdentity accountIdentity)
    {
        ...
        if (!saveResult.Succeeded)
            return _notificationHandler.CreateNotification("Identity", "Dados de credenciais Login/Senha com inconsistência.");
        else
            return true;
    }
    ...
    public async Task<bool> SignPasswordAsync(UserLogin login)
    {
        ...
        if (!loginResult.Succeeded)
            return _notificationHandler.CreateNotification("Identity", "Login ou senha inválidos.");
